...
| ⭐ `lectura-cortada-por-max-rows` | "Se guardó pero al recargar no está": PostgREST corta en 1.000 filas **sin error** y faltan las más nuevas. Diagnóstico (logs de clave duplicada + conteo por tenant + prueba con control positivo), el helper `leerTodo` paginado con orden total, y por qué `.limit()` y subir `max_rows` no lo arreglan |
