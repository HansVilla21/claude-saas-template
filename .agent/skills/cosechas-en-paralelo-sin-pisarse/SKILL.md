...
### Y después esta skill se perdió, exactamente así

La primera versión de este archivo se commiteó y se pushó a una rama **cuyo PR ya había sido mergeado un minuto antes**. El commit quedó en la rama, la rama quedó verde, el PR decía `MERGED`, y la skill **no llegó a `main`**. Se detectó recién al verificar los archivos contra el disco después del merge — el paso 3 de esta misma skill.

Es el pariente directo de `verificar-base-del-pr-antes-de-mergear`: **un push exitoso a una rama no es un cambio entregado.** Cuando hay varias sesiones mergeando, la ventana entre "pusheo" y "alguien mergea" deja de ser teórica.

```bash
# Después de pushear a una rama con PR abierto, confirmá que sigue abierto:
gh pr view <n> --json state,mergedAt
# Y después de cualquier merge, confirmá que tu trabajo está EN main:
git merge-base --is-ancestor <tu-commit> origin/main && echo "llegó" || echo "NO llegó"
```
