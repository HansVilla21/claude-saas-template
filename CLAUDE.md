...
├── .agent/
│   └── skills/        141 skills de proceso reusables:
...
│                      reportadas como bugs distintos) +
│                      ⭐ manual-de-ayuda-dentro-del-producto (el manual VIVE
│                      en el producto —ruta /ayuda— y su
...
│                      número de arriba queda mintiendo—. Y esta skill se
│                      PERDIÓ exactamente así: su primer commit se pushó a una
│                      rama cuyo PR ya se había mergeado un minuto antes —rama
│                      verde, PR MERGED, archivo ausente de main—; se detectó
│                      al verificar contra el disco después del merge. Las 3
│                      reglas: el número se reclama en el PR (gana quien lo
│                      abrió primero) no en el archivo; el conteo se DERIVA con
│                      `ls | wc -l` después de cada merge, nunca de memoria; y
│                      el índice se VERIFICA contra el disco por grep —chequeo
│                      que además atrapa un nombre partido en dos líneas por el
│                      ancho de columna, que deja de resolver—. Al resolver: en
│                      un índice acumulativo la resolución correcta es `ours +
